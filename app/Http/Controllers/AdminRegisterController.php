	...
	class AdminRegisterController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    */
	    public function hook_after_edit($id) {
	         $register=DB::table('register')->where('id',$id)->first();       
	        CRUDBooster::redirect('/admin/register/add/?&registerid='.$register->eventid,"Вами було змінено реєстрацію для ".$register->name,"success");  // на сторінку з повідомленням   // на сторінку з повідомленням  // на сторінку з повідомленням 
...
public function getAdd($id=0) {
  ...
  if ($_GET['clubid']) {
  	...
  	$data['registers'] = DB::table('register')->orderBy('created_at', 'DECK')->where('eventid',$_GET['registerid'])->where('club',$club->title)->get();
  }
  else{

  $data['registers'] = DB::table('register')->orderBy('created_at', 'DECK')->where('eventid',$_GET['registerid'])->where('userid',CRUDBooster::myId())->get(); //Зареєстровані 
  }
  ...
  $this->cbView('site.addregister',$data);
}
