...
class AdminEvent19Controller extends \crocodicstudio\crudbooster\controllers\CBController {
	...
	public function cbInit() {
		...
		$this->col[] = ["label"=>"Назва","name"=>"location"];
		...
		*/
		...
		public function getAdd($id=0) {
			...
			$this->cbView('site.add_event',$data);
		} 
  ...
  //Create an Auth
			...
			$data = [];
			$data['dani']=DB::table('event')->where('id',$id)->first();
  ...
  //Create an Auth
			...
			$data['row'] = DB::table('event')->where('id',$id)->first();
  ...
  //Please use cbView method instead view method from laravel
			$this->cbView('site.detail_event',$data);
		}
