...
@if($registerseting->club)
    <div class="form-group col-md-1">
      <label for="inputZip">Команда-клуб</label>
...
@if($registerseting->si)
    <div class="form-group col-md-1">
      ...
      <input type="text" class="form-control" id="si"  value="{{$row->si}}" name='si'required>
    </div>
@endif
...
@if($registerseting->si)
    <div class="form-group col-md-1">
      ...
      <select  class="form-control" id="dni" value="{{$row->dni}}" name='dni'required>@foreach($registerseting->dni as $dni)
        <!-- <option selected>1-3</option> -->
        <option>{{$dni}}</option>
      @endforeach
      </select>
